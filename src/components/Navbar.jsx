            ...
            <div className="space-y-2">
                {displayData.map((item, index) => (
                    <Link
                        key={index}
                        to={item.link}
                        className="flex gap-4 items-center p-2 rounded-md cursor-pointer w-full hover:bg-gradient-to-r hover:from-blue-300 hover:via-white hover:to-purple-400 transform hover:scale-110 transition-all ease-in-out"
                    >
                        <img
                            src={item.src}
                            className={`size-8 object-cover ${index == 0 && 'rounded-full'}`}
                        />
                        <span className="font-medium text-[15px] text-gray-800 group-hover:text-white">{item.text}</span>
                    </Link>
                ))}
                {/* Toggle Button */}
                <button
                    className={`flex gap-4 items-center p-2 rounded-md cursor-pointer w-full transform hover:scale-110 transition-all ease-in-out ${showAll ? "bg-purple-200 hover:bg-purple-300" : "bg-blue-200 hover:bg-blue-300"
                        }`}
