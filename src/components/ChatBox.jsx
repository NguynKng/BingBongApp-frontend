  ...
  <div className="rounded-t-xl shadow-xl overflow-hidden bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <div>
          <Link to={`/profile/${userChat._id}`} className="text-base text-white">
            {`${userChat.fullName}`}
          </Link>
          {isOnline && (
            <span className="text-green-300 block text-[13px]">Đang hoạt động</span>
          )}
        </div>
      </div>
      <div className="flex items-center gap-2">
    ...
    {/* Chat content */}
    <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isMinimized ? "max-h-0 opacity-0" : "max-h-[500px] opacity-100"}`}>
      <div className="flex flex-col gap-2 p-2 min-h-72 max-h-96 overflow-y-auto text-sm dark:text-gray-200">
        {messages.map((msg) => {
          ...
          return (
            <div
              key={msg._id}
              className={`flex gap-2 max-w-[75%] ${isMyMessage ? "self-end flex-row-reverse" : "self-start"}`}
            >
              {!isMyMessage && (
                <img
                  src={userChat.avatar ? `${Config.BACKEND_URL}${userChat.avatar}` : "/user.png"}
