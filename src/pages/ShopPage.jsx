...
export default function ShopPage() {
  ...
  return (
    <>
      <div className="lg:px-[15%] bg-gray-100 dark:bg-[#181826]">
        <div className="relative w-full">
          <div className="relative w-full lg:h-[24rem] md:h-[22rem] sm:h-[20rem] h-[18rem] rounded-b-md">
            <img
              src={Config.BACKEND_URL + shop.cover}
            ...
            {isMyShop && (
              <div className="absolute bottom-4 right-4 z-31 flex items-center gap-2 bg-white hover:bg-gray-300 cursor-pointer rounded-md py-2 px-4 text-black font-medium">
      ...
      <section className="bg-gray-200 dark:bg-[#181826] lg:px-[15%] px-2 py-4 min-h-screen">
        <Routes>
          <Route path="/" element={<PostTab shop={shop} user={user} />} />
          <Route
            path="product"
            element={<ProductTab shop={shop} user={user} />}
          />
          <Route path="about" element={<AboutTab shop={shop} user={user} />} />
          <Route path="photos" element={<PhotoTab shop={shop} user={user} />} />
          {isMyShop && (
            <Route
              path="manage/*"
              element={<ManageTab shop={shop} user={user} />}
            />
          )}
        </Routes>
      </section>
